...
% {
%disp(sprintf('\nTest vs builtin:  \teig_thr Eigenvalues\teig Eigenvalues  \tEigenvectors differences'));
%[V1,E1]=eig_thr(dsy); [V2,E2]=eig(dsy); disp(sprintf('Real symmetric   \t%10.5f\t\t%10.5f\t\t%10.5f\n',[diag(E1),diag(E2),sum(abs(V1)-abs(V2),2)]'));
%[V1,E1]=eig_thr(zhe); [V2,E2]=eig(zhe); disp(sprintf('Complex hermitian\t%10.5f\t\t%10.5f\t\t%10.5f\n',[diag(E1),diag(E2),sum(abs(V1)-abs(V2),2)]'));
%[V1,E1]=eig_thr(dge); [V2,E2]=eig(dge); disp(sprintf('Real general     \t%10.5f +i%10.5f\t%10.5f +i%10.5f\t%10.5f\n',[real(diag(E1)),imag(diag(E1)),real(diag(E2)),imag(diag(E2)),sum(abs(V1)-abs(V2),2)]'));
%[V1,E1]=eig_thr(zge); [V2,E2]=eig(zge); disp(sprintf('Complex general  \t%10.5f +i%10.5f\t%10.5f +i%10.5f\t%10.5f\n',[real(diag(E1)),imag(diag(E1)),real(diag(E2)),imag(diag(E2)),sum(abs(V1)-abs(V2),2)]'));
disp(sprintf('\nTest vs builtin:  \tEigenvalues diffs  \tEigenvectors differences'));
disp(sprintf('------------------------------------------------------------------------'));
[V1,E1]=eig_thr(dsy); [V2,E2]=eig(dsy); disp(sprintf('Real symmetric   \t% 10.5g\t\t% 10.5g',abs(sum(diag(E1)-diag(E2))),sum(sum(abs(V1)-abs(V2)))));
[V1,E1]=eig_thr(zhe); [V2,E2]=eig(zhe); disp(sprintf('Complex hermitian\t% 10.5g\t\t% 10.5g',abs(sum(diag(E1)-diag(E2))),sum(sum(abs(V1)-abs(V2)))));
[V1,E1]=eig_thr(dge); [V2,E2]=eig(dge); disp(sprintf('Real general     \t% 10.5g\t\t% 10.5g',abs(sum(diag(E1)-diag(E2))),sum(sum(abs(V1)-abs(V2)))));
[V1,E1]=eig_thr(zge); [V2,E2]=eig(zge); disp(sprintf('Complex general  \t% 10.5g\t\t% 10.5g',abs(sum(diag(E1)-diag(E2))),sum(sum(abs(V1)-abs(V2)))));
disp(sprintf('\nConsistency tests:\tV*E*inv(V)-A\tV*E*V''-A\tV*V''-I\t\tV''*V-I'))
disp(sprintf('------------------------------------------------------------------------------'));
[V,E]=eig_thr(dsy); disp(sprintf('Real symmetric   \t% 10.4g\t% 10.4g\t% 10.4g\t% 10.4g',sum(sum(V*E*inv(V)-dsy)),sum(sum(V*E*V'-dsy)),sum(sum(V*V'-eye(n))),sum(sum(V'*V-eye(n)))));
[V,E]=eig_thr(zhe); disp(sprintf('Complex hermitian\t% 10.4g\t% 10.4g\t% 10.4g\t% 10.4g',sum(sum(V*E*inv(V)-zhe)),sum(sum(V*E*V'-zhe)),sum(sum(V*V'-eye(n))),sum(sum(V'*V-eye(n)))));
[V,E]=eig_thr(dge); disp(sprintf('Real general     \t% 10.4g\t% 10.4g\t% 10.4g\t% 10.4g',sum(sum(V*E*inv(V)-dge)),sum(sum(V*E*V'-dge)),sum(sum(V*V'-eye(n))),sum(sum(V'*V-eye(n)))));
[V,E]=eig_thr(zge); disp(sprintf('Complex general  \t% 10.4g\t% 10.4g\t% 10.4g\t% 10.4g',sum(sum(V*E*inv(V)-zge)),sum(sum(V*E*V'-zge)),sum(sum(V*V'-eye(n))),sum(sum(V'*V-eye(n)))));
%}
...
for tp=1:4
 ...
 %end;
  ...
  tic; [V2,E2]=eig_thr(B,'orth','sort'); t2(tp)=toc;
...
disp(sprintf('\nOrth Test:  \t\tEigenvecDiffs   EigenvalDiffs   Time(eigorth)   Time(eig_thr)         Speedup'));
disp(sprintf('-----------------------------------------------------------------------------------------------------'));
...
disp(sprintf('\nSpeed Test:  \t\tTime(eig_thr)\t   Time(eig)\t   Speedup    Eigenvalue diffs   Sum(V*E*V''-A)\tSum(V''*V-I)'));
disp(sprintf('-------------------------------------------------------------------------------------------------------------------'));
...
for tp=1:4
  ...
  tic; if(tp>2); K2 = chol_thr(mm,'lower'); else; K2 = chol_thr(mm); end; t2(tp)=toc;
...
for tp=5:8
  ...
  tic; if(tp>6); [~,p2] = chol_thr(mm,'lower'); else; [~,p2] = chol_thr(mm); end; t2(tp)=toc;
  ch(tp) = sum(sum(p1-p2));
end
disp(sprintf('\nCholesky Test:  \tTime(chol_thr)\t   Time(chol)\t   Speedup    CholFactor diffs   PositiveDef diffs'));
disp(sprintf('------------------------------------------------------------------------------------------------------------------'));
...
for tp=1:4
  ...
  tic; if(tp>2); [K2,inv2] = chol_thr(mm,'Colpa','lower'); else; [K2,inv2] = chol_thr(mm,'Colpa'); end; t2(tp)=toc;
  ch(tp) = sum(sum(sum(abs(K1-K2))));
  ch(tp+4) = sum(sum(sum(abs(inv1-inv2))));
end
disp(sprintf('\nColpa Test:     \tTime(chol_thr)\t   Time(chol+eig)  Speedup    K*g*K'' diffs\t inv(K) diffs'));
disp(sprintf('------------------------------------------------------------------------------------------------------------------'));
