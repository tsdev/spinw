...
while 1 % main messaging loop
    ...
    try
        ...
        if isdeployed
            ...
            % other errors deeper in the code
            for ii = 1:(numel(errStr.stack)-2)
            ...
            % if not all m-files, do not show the error stack
            if ~allm
                for ii = 1:numel(errStr.stack)
                    errStr.stack(ii).file(end) = 'p';
                end
            end
