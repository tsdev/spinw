function spectra = twomagnon3(obj, hkl, varargin)
...
for ii = 1:nHkl
    for jj = 1:nKm
        ...
        randQ = rand(3,nRand);
        %randQ = rand(2,nRand);
        %randQ = [randQ; zeros(1, nRand)]; %#ok<AGROW>
        
        % calculate dispersion, V matrix
        [omega1, V1] = obj.corespec(randQ);
        %[omega2, V2] = obj.corespec(bsxfun(@minus,hkl(:,ii)+kmList(:,jj),randQ));
        [omega2, V2] = obj.corespec(bsxfun(@plus,-randQ,hkl(:,ii)+kmList(:,jj)));
        
        omega1 = permute(omega1(1:nMode/2,:),[1 3 2]);
        omega2 = permute(omega2(1:nMode/2,:),[3 1 2]);
        
        % energy of the two magnon process = sum(omega)
        omega = bsxfun(@plus,omega1,omega2);
        
        % intensity
        % dimensions [nMode nMode nRand alpha beta]
        ExpF1 = bsxfun(@times,ExpF(:,:,ii),permute(eta,[3 2 4 1]));
        
        Int = mmat(bsxfun(@times,ExpF1,permute(conj(V1(1:nMode/2,1:nMode/2,:)),[2 1 3])),V2(1:nMode/2,1:nMode/2,:));
        %Int = mmat(permute(conj(V1(1:nMode/2,1:nMode/2,:)),[1 2 3]),bsxfun(@times,ExpF1,V2(1:nMode/2,1:nMode/2,:)));
...
%         end
        ...
        Int = permute(reshape(bsxfun(@times,Int,nBose),(nMode/2)^2,1,nRand,3,3),[4 5 1 3 2]);
        omega = permute(reshape(omega,(nMode/2)^2,1,nRand),[1 3 2]);
        
        [~, idxE] = min(abs(repmat(real(omega),[1 1 nE])-repmat(permute(Evect,[2 3 1]),[(nMode/2)^2 nRand 1])),[],3);
