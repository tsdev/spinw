function spectra = twomagnon2(obj, hkl, varargin)
...
ExpF = repmat(exp(1i*sum(repmat(permute(hklExt,[1 3 2]),[1 nMode/2 1]).*repmat(RR,[1 1 nHkl]),1)),[1 2 1]);
...
for ii = 1:nHkl
    for jj = 1:nKm
        ...
        omega1 = permute(omega1,[1 3 2]);
        omega2 = permute(omega2,[3 1 2]);
        ...
        ExpF1 = bsxfun(@times,ExpF(:,:,ii),repmat(permute(eta,[3 2 4 1]),[1 2 1 1]));
        Int = mmat(bsxfun(@times,ExpF1,V2),permute(conj(V1),[2 1 3]));
        ...
        Int = permute(reshape(bsxfun(@times,Int,nBose),nMode^2,1,nRand,3,3),[4 5 1 3 2]);
        omega = permute(reshape(omega,nMode^2,1,nRand),[1 3 2]);
